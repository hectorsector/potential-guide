...
//   parseInt(process.env.EXTERNAL_LINK_CHECKER_MAX_AGE_DAYS || '7') *
//   24 *
//   60 *
//   60 *
//   1000
...
//     core.info(
//       `Checking of external links is is cached to ${EXTERNAL_LINK_CHECKER_DB}`
//     )
//     core.info(
...
//     core.info(
//       `Checked ${(globalCacheHitCount + globalCacheMissCount).toLocaleString()} links`
//     )
//   }
...
//     core.warning(
//       `commentOnPR called outside of PR action runner context. Not creating comment.`
//     )
//     return
//   }

//   const findAgainSymbol =
//     '<!-- rendered-content-link-checker-comment-finder -->'
...
//       hrefsOnPageGroup[page.fullPath][linkKey] = {
//         href,
//         text,
//         src,
//         flaw,
...
//     if (
//       !hrefsOnPageGroup[pageKey][linkKey].permalinkHrefs.includes(
//         permalink.href
//       )
//     ) {
...
//         const countDOMItems =
//           $(href).length + $(`a[name="${href.slice(1)}"]`).length
//         if (countDOMItems === 0) {
...
//         return {
//           WARNING: `Redirect to ${getRedirect(pathname, { redirects, pages: pageMap })}`,
//         }
...
//       return
//     }
//     const { ok, ...info } = await checkExternalURLCached(
//       core,
//       href,
//       { verbose, patient },
//       db
...
//       }
//       return {
//         [problem]: `Broken external link (${JSON.stringify(info)})`,
//         isExternal: true,
//       }
//     }
//   }
...
//     if (
//       (r.statusCode === 405 || r.statusCode === 404 || r.statusCode === 403) &&
//       !useGET
//     ) {
...
//   )
//   core.info(
//     `Took ${Math.floor(tookSeconds)} seconds. (~${(tookSeconds / 60).toFixed(1)} minutes)`
//   )
...
//   const guts = [page.rawIntro, page.rawPermissions, page.markdown]
//     .filter(Boolean)
//     .join('\n')
//     .trim()
